import { createCloudflareRadarAdapter, decodeRadarTopicIdPathSegment } from '../../../_shared/radar/RadarAdapter';
...
const handleGet: PagesFunction<RadarFunctionEnvironment, 'topicId'> = async (context) => {
  try {
    const topicId = decodeRadarTopicIdPathSegment(String(context.params.topicId ?? ''));
