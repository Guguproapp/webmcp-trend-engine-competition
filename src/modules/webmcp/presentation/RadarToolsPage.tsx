...
export function RadarToolsPage({ gateway: suppliedGateway }: { gateway?: RadarBrowserGateway }) {
  ...
  async function search(event: React.FormEvent) {
    event.preventDefault(); setState('loading'); setMessage('正在讀取熱門雷達。');
    try {
      const data = await gateway.trends({ market: market as RadarMarket, ...(category ? { category } : {}), type: type as RadarType, hours, ...(minConfidence > 0 ? { minConfidence } : {}), ...(source ? { source } : {}), sort: sort as RadarSort, limit });
  ...
  return <section className="radar-workspace" aria-labelledby="radar-tools-title">
    <div className="radar-hero"><div><span>READ-ONLY WEBMCP · 唯讀工具</span><h1 id="radar-tools-title">熱門雷達工具 <small>Asia Trend Radar Tools</small></h1><p>代理人與一般網站共用同一個伺服器端雷達轉接器；只查詢，不修改排程、來源或資料。</p></div><strong className={`radar-native-state state-${nativeStatus}`}>{nativeStatus === 'supported' ? '6 個唯讀工具已就緒' : nativeStatus === 'checking' ? '正在檢查 WebMCP' : nativeStatus === 'unsupported' ? '安全降級：一般搜尋可用' : 'WebMCP 註冊失敗；一般搜尋可用'}</strong></div>

    <div className="radar-proof-grid" aria-label="WebMCP工具契約">
    ...
    <section className="radar-panel" aria-labelledby="fallback-search-title">
      <div className="radar-panel-heading"><div><span>WEBSITE FALLBACK</span><h2 id="fallback-search-title">一般網站搜尋備援</h2></div><p>即使瀏覽器不支援 WebMCP，仍使用同一套資料與錯誤語意。</p></div>
