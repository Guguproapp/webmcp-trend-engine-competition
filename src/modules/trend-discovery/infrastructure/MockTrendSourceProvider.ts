...
export const MOCK_TOPIC_CONFIGS: MockTopicConfig[] = [
  { key:'subscription-fatigue', title:'Mock 測試｜訂閱服務悄悄漲價引發「訂閱疲勞」', summary:'消費者開始整理每月自動扣款，分享取消與降級經驗。', category:'生活消費', keywords:['訂閱','漲價','省錢'], platforms:['threads','youtube','google_trends','news_rss'], hoursAgo:.4, metrics:[95,96,93,92,95,96,22,8,80,95] },
  ...
  { key:'shrinkflation', title:'Mock 測試｜同價縮量的生活用品被集體比較', summary:'網友用舊包裝對照新規格，帶動價格透明討論。', category:'社會共鳴', keywords:['縮水式通膨','物價','比較'], platforms:['threads','youtube','news_rss'], hoursAgo:1.2, metrics:[92,91,94,87,90,98,28,12,68,93] },
  ...
  { key:'family-screen-time', title:'Mock 測試｜家庭螢幕時間協議', summary:'家長與孩子共同制定手機使用規則，正反經驗快速增加。', category:'親子家庭', keywords:['親子','手機','螢幕時間'], platforms:['threads','youtube','news_rss'], hoursAgo:3, metrics:[79,86,84,74,82,92,35,18,64,86] },
  { key:'concert-queue', title:'Mock 測試｜演唱會排隊規則爭議', summary:'購票與入場體驗引發粉絲社群討論，但熱度生命週期偏短。', category:'娛樂話題', keywords:['演唱會','排隊','購票'], platforms:['threads','youtube','news_rss'], hoursAgo:1.8, metrics:[86,88,79,81,88,90,48,30,42,88] },
  { key:'lunch-price', title:'Mock 測試｜百元午餐消失感', summary:'外食族分享日常午餐價格變化與替代選擇。', category:'美食餐飲', keywords:['午餐','外食','物價'], platforms:['threads','google_trends','news_rss'], hoursAgo:4, metrics:[80,78,85,72,78,97,42,12,72,91] },
  ...
  { key:'delivery-fee', title:'Mock 測試｜外送費顯示方式爭議', summary:'消費者比較結帳前後價格，服務透明度成為焦點。', category:'爭議事件', keywords:['外送','費用','透明'], platforms:['threads','youtube','news_rss'], hoursAgo:6, metrics:[72,66,76,64,70,92,54,38,48,84] },
  ...
  { key:'expired-coupon', title:'Mock 測試｜已結束的限時優惠討論', summary:'活動已過期，剩餘搜尋與互動快速下降。', category:'生活消費', keywords:['優惠','過期'], platforms:['threads','news_rss'], hoursAgo:90, metrics:[32,18,30,26,18,80,76,8,6,82] },

  { key:'dangerous-challenge', title:'Mock 測試｜高風險模仿挑戰擴散', summary:'互動快速增加但可能造成身體傷害，必須獨立標示高風險。', category:'爭議事件', keywords:['挑戰','危險','模仿'], platforms:['threads','youtube','news_rss'], hoursAgo:1, metrics:[92,94,86,88,94,90,36,92,36,92] },
  { key:'unverified-rumor', title:'Mock 測試｜未查證名人傳聞快速擴散', summary:'來源互相引用且可能傷害當事人，不應因流量高而推薦。', category:'娛樂話題', keywords:['傳聞','未查證','名人'], platforms:['threads','youtube','news_rss'], hoursAgo:1.5, metrics:[94,90,78,86,92,82,44,88,28,80] },
  ...
  { key:'heavy-rain-info', title:'Mock 測試｜豪雨即時資訊大量轉貼', summary:'新聞量高但主要是氣象與交通資訊，缺少情緒共鳴與延伸角度。', category:'其他', keywords:['豪雨','天氣','交通'], platforms:['threads','news_rss','google_trends'], hoursAgo:.4, metrics:[96,90,42,76,96,98,18,24,18,91], natural:true, naturalEvidence:allNaturalFalse },
  { key:'earthquake-info', title:'Mock 測試｜地震速報搜尋瞬間上升', summary:'瞬時搜尋很高，但內容以速報為主且生命週期極短。', category:'其他', keywords:['地震','速報','自然災害'], platforms:['threads','news_rss','google_trends','youtube'], hoursAgo:.2, metrics:[99,98,38,88,99,99,20,36,8,94], natural:true, naturalEvidence:{...allNaturalFalse, emotionalResonance:true} },
];

const providerNames = ['Threads Keyword Search API', 'YouTube Data API', 'Google Trends', '新聞 RSS', '客戶授權帳號資料', '競爭者與關鍵字追蹤'];
  ...
  async collectSignals(request: TrendCollectionRequest): Promise<RawTrendSignal[]> {
    ...
    return MOCK_TOPIC_CONFIGS.flatMap((config) => config.platforms.map((platform, index) => {
      ...
      return {
        ...
        sourceItem: {
          id:`mock-${config.key}-${platform}`, platform, title:`${config.title}｜${platform} Mock 證據`, discoveredAt, publishedAt,
          engagementCount:Math.round(currentHeat * (850 + index * 240)), growthDelta:growthRate - 40 + index * 2,
        ...
        currentHeat, growthRate, freshness, crossPlatformResonance, socialResonance, taiwanRelevance,
