...
function TrendListPage({ fullFilters = false }: { fullFilters?: boolean }) {
  ...
  const overviewStats = [
    ...
    <Stat key="sources" label="資料來源數" value={`${latest?.sourceCount ?? trendDiscoveryService.getProviderNames().length}`} />,
    <Stat key="topics" label="蒐集主題數" value={`${topics.length}`} />,
    <Stat key="potential" label="高潛力主題" value={`${highPotentialTopics.length}`} highlight />,
  ];
  const searchStats = [
    <Stat key="updated" label="上次更新" value={latest ? formatDateTime(latest.refreshedAt) : '準備中'} />,
    <Stat key="sources" label="資料來源數" value={`${latest?.sourceCount ?? trendDiscoveryService.getProviderNames().length}`} />,
