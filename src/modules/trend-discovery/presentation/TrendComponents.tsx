...
export function TrendTopicCard({ topic, rank, watching, onWatch, onRemoveWatch, onExclude, onCancelExclude, excludedReason }: { topic:TrendTopic; rank?:number; watching:boolean; onWatch:()=>void; onRemoveWatch:()=>void; onExclude?:(reason:ExclusionReason)=>void; onCancelExclude?:()=>void; excludedReason?:string }) {
  ...
  return <article className={`trend-topic-card tier-${topic.tier}`}>
    <div className="trend-card-top"><div className="trend-card-labels">{rank && rank <= 3 && <span className="rank-label">第 {rank} 名</span>}<span className="trend-category">{topic.category}</span><span className={`trend-tier tier-label-${topic.tier}`}>{TREND_TIER_LABELS[topic.tier]}</span>{watching && <span className="watching-flag">觀察中</span>}{topic.status === 'high_risk' && <span className="risk-flag">警告：高風險</span>}{topic.status === 'insufficient_evidence' && <span className="evidence-flag">證據不足</span>}<span className="mock-data-label">Mock 資料</span></div><ScoreRing score={topic.totalScore} /></div>
