...
export function TrendFilterPanel({ filters, onApply, onClear, onSave, compact=false }: { filters: TrendFilters; onApply: (filters: TrendFilters) => void; onClear: () => void; onSave: (filters: TrendFilters) => void; compact?:boolean }) {
  ...
  return <section className={`trend-filter-panel${compact?' trend-filter-panel--compact':''}`} aria-label="熱門主題篩選器">
    ...
    {!compact && <details open={advancedOpen} onToggle={(event) => setAdvancedOpen(event.currentTarget.open)} className="advanced-filters">
