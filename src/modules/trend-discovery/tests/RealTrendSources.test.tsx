...
describe('公開介面真實資料邊界',()=>{
  it('D1 Repository涵蓋六張必要資料表、參數綁定與批次保存',()=>{for(const table of ['trend_topics','trend_signals','trend_topic_signals','trend_snapshots','trend_provider_runs','trend_refresh_locks'])expect(migration).toContain(`CREATE TABLE IF NOT EXISTS ${table}`);expect(repositorySource).toContain('.bind(');expect(repositorySource).toContain('this.db.batch');});
