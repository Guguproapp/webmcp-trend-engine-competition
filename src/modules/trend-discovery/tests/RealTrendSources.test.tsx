...
describe('公開介面真實資料邊界',()=>{
  ...
  it('手機主要導覽固定為雷達、熱門、搜尋、更多且觀察清單仍可由更多進入',async()=>{render(<MemoryRouter initialEntries={['/trends']}><App/></MemoryRouter>);const nav=screen.getByRole('navigation',{name:'手機主要導覽'});expect(nav.querySelectorAll(':scope > a, :scope > button')).toHaveLength(4);expect(nav).toHaveTextContent('雷達');expect(nav).toHaveTextContent('熱門');expect(nav).toHaveTextContent('搜尋');expect(nav).toHaveTextContent('更多');fireEvent.click(screen.getByRole('button',{name:/更多/}));await waitFor(()=>expect(screen.getByRole('menuitem',{name:/觀察清單/})).toHaveAttribute('href','/trends/watchlist'));});
  it('更多選單具有展開語意、焦點移入並可用Escape關閉後回焦',async()=>{render(<MemoryRouter initialEntries={['/trends']}><App/></MemoryRouter>);const more=screen.getByRole('button',{name:/更多/});fireEvent.click(more);expect(more).toHaveAttribute('aria-expanded','true');const first=screen.getByRole('menuitem',{name:/產品首頁/});await waitFor(()=>expect(first).toHaveFocus());fireEvent.keyDown(screen.getByRole('menu'),{key:'Escape'});await waitFor(()=>expect(more).toHaveFocus());expect(more).toHaveAttribute('aria-expanded','false');});
